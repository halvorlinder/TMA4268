...
Where:
 ...
 - $(f(x_0) -E[\hat{f}(x_0)])^2$ is the bias if the prediction
...
The KNN model, however, only has an AUC value of 0.8417, implying that it performs quite a bit worse on predicting the test data.
...
penguin.glm.plot
```
The plot show the true species (the colored dots) and our prediction boundary. All dots that fall below the boundary would be classifies as 1 (adelie), while dots above the boundary would be classified as 0 (not adelie). 
$$
\text{The formula directly follows from the definition of the logistic model:} \\
\log{\frac{p}{1-p}}=\beta_0 + \beta_1 x_1 + \beta_2 x_2 \\
\text{Reordering the terms, we can express the relation between } x_1 \text{ and } x_2 \text{ linearly:} \\
x_2 = \frac{\log{\frac{p}{1-p}} - \beta_0}{\beta_2} - \frac{\beta_1 x_1}{\beta_2} \\
\text{We previously set the boundary at } p=1/2 \text{. Inserting this gives:} \\
