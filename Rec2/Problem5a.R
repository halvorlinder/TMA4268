...
#------
# create sequence of sampling points 
...
# produce f(x) values for the list of xs
true_y <- truefunc(x)
# create random errors for M observations at each value of x
error <- matrix(rnorm(length(x) * M, mean = 0, sd = 2),
                nrow = M,
                byrow = TRUE)
# combine the errors with the errors in some way 
ymat <- matrix(rep(true_y, M), byrow = T, nrow = M) + error # Each row is a simulation
#------
# create a matrix containing the predictions for each polynomial degree
...
typeof(predictions_list)
head(predictions_list[1])
# Plotting -----
...
ggplot(plotting_df, aes(x = x, y = y, group = simulation_num)) +
  ...
  geom_line(aes(x = x, y = x*x), size = 1.5) +
